 ...
 */
export function initGlobalMocks(config) {
    const g = global;
    if (!g.fs)
        g.fs = safeFs;
    if (!g.path)
    ...
    try {
        ...
        if (!g.canvas)
            g.canvas = canvasModule;
        const registerFont = canvasModule.registerFont ||
            (canvasModule.GlobalFonts
                ? (fontPath, options) => canvasModule.GlobalFonts.registerFromPath(fontPath, options.family)
                : null);
        if (registerFont) {
            if (!g.canvas.registerFont)
                g.canvas.registerFont = registerFont;
            if (!g.registerFont)
                g.registerFont = registerFont;
        }
