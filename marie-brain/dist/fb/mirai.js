...
if (!g.client) {
    ...
    if (!g.client.handleReply)
        g.client.handleReply = [];
    if (!g.client.handleEvent)
        g.client.handleEvent = [];
}
if (!g.fs)
    g.fs = safeFs;
if (!g.data) {
...
function buildGetText(rawModule) {
    return (key, ...args) => {
...
export function wrapMiraiCommand(rawModule) {
    ...
    try {
        if (rawModule.onLoad)
            rawModule.onLoad({ configModule: global.configModule });
    }
    catch (e) {
        // silent
    }
    const handler = async (ctx) => {
        ...
        try {
            ...
            const replyEntry = ctx.event.messageReply
                ? replyList.find((r) => r.messageID === ctx.event.messageReply?.messageID)
                : null;
