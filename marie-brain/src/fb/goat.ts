...
if (!g.GoatBot) {
  ...
  if (!g.GoatBot.cooldowns) g.GoatBot.cooldowns = new Map();
}
if (!g.temp) g.temp = {};
if (!g.fs) g.fs = safeFs;
...
if (!g.utils) {
  g.utils = {
    getPrefix: (threadID?: string) => (global as any).config?.PREFIX || '.',
    getTime: () => Date.now(),
    drive: {}
  };
}
...
const mockThreadsData = {
  get: async (id: string) => ({ threadName: `Thread ${id}`, settings: {}, members: [] }),
  set: async (id: string, data: any) => true,
  getAll: async () => []
};

const mockUsersData = {
  get: async (id: string) => ({ name: `User ${id}` }),
  getName: async (id: string) => `User ${id}`,
  set: async (id: string, data: any) => true,
  getAll: async () => []
};
 ...
 */
function replacePn(text: any): any {
  const prefix = (global as any).config?.PREFIX || '.';
  if (typeof text === 'string') return text.replace(/\{pn\}/g, prefix);
 ...
 */
export function wrapGoatCommand(rawModule: any): ICommand {
  const config = rawModule.config || {};
  let minRole: 'user' | 'admin' | 'owner' = 'user';
  if (config.role === 1) minRole = 'admin';
  if (config.role === 2) minRole = 'owner';

  const handler = async (ctx: IMarieContext) => {
    // Cooldown enforcement
    const cooldownTime = (config.countDown || config.cooldown || 0) * 1000;
    if (cooldownTime > 0) {
      const cooldowns = (global as any).GoatBot.cooldowns;
    ...
    const rawApi = (ctx.platform as any).api || ctx.platform;
    const api = new Proxy(rawApi, {
      get(target, prop: string) {
        if (prop === 'sendMessage') {
          return (body: any, threadID: any, ...rest: any[]) => {
    ...
    const getLang = (key: string, ...args: any[]) => {
    ...
    try {
      const hook = (ctx as any)._hookCall;
...
export function wrapGoatEvent(rawModule: any) {
  ...
  return async (ctx: IMarieContext) => {
    ...
    const goatCtx = {
      api: (ctx.platform as any).api || ctx.platform,
      event: ctx.event,
      args,
      message,
      threadsData: mockThreadsData,
    ...
    try {
      if (rawModule.onChat) await rawModule.onChat(goatCtx);
      if (rawModule.onEvent) await rawModule.onEvent(goatCtx);
      if (rawModule.onAnyEvent) await rawModule.onAnyEvent(goatCtx);
    } catch (e: any) {
