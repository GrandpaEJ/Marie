...
class AnimeImageService {
  async getImage(type = 'sfw', category = 'waifu') {
    ...
    try {
      ...
      if (data.url) return { url: data.url, source: 'waifu.pics' };
    } catch (e) {
    ...
    if (type === 'sfw') {
      try {
        ...
        const data = await res.json();
        if (data.results?.[0]?.url) return { url: data.results[0].url, source: 'nekos.best' };
      } catch (e) {
    ...
    if (type === 'sfw') {
      try {
        ...
        const data = await res.json();
        if (data.url) return { url: data.url, source: 'nekos.life' };
      } catch (e) {
    ...
    try {
      ...
      const data = await res.json();
      if (data.url) return { url: data.url, source: 'nekosapi.com' };
    } catch (e) {
