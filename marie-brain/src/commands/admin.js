...
export default {
    ...
    handler: async (ctx) => {
        ...
        if (!args[0])
            return api.sendMessage("[Marie] See .help admin for usage.", threadID);
        ...
        if (sub === 'role') {
            if (user.role !== 'owner')
                return api.sendMessage("[Marie] Only owner can change roles.", threadID);
            const targetUid = args[1];
            const targetRole = args[2]?.toLowerCase();
            if (!targetUid || !targetRole)
                return api.sendMessage("[Marie] Usage: .admin role <uid> <role>", threadID);
            try {
        ...
        if (sub === 'restart') {
            if (user.role !== 'owner')
                return api.sendMessage("[Marie] Only owner can restart the bot.", threadID);
